...
class GuideOverlayView @JvmOverloads constructor(
    ...
    @ColorInt
    private var overlayColor = Color.parseColor("#99000000")
     ...
     * @param radius 圆角半径，默认为0（直角矩形）
     */
    fun setHoleForView(targetView: View, padding: Float = 0f, radius: Float = 0f) {
        ...
        // 计算相对位置
        val rect = RectF(
            (location[0] - selfLocation[0] - padding).toFloat(),
            (location[1] - selfLocation[1] - padding).toFloat(),
            (location[0] - selfLocation[0] + targetView.width + padding).toFloat(),
            (location[1] - selfLocation[1] + targetView.height + padding).toFloat()
        )
