 ...
 * 文生图任务执行器
 */
class TextToImageExecutor : TaskExecutor() {
    ...
    private suspend fun processTask(task: AITaskInfo, statusListener: TaskListener) {
        LogUtils.d(TAG, "开始处理文生图任务: ${task.taskId}")
     ...
     * 处理任务成功
     */
    private suspend fun handleTaskSuccess(
        task: AITaskInfo,
        resultUrl: String,
        statusListener: TaskListener
    ) {
        LogUtils.d(TAG, "文生图任务执行成功，开始下载结果: ${task.taskId}")
        ...
        FileDownloadHelper.downloadFile(
            ...
            LogUtils.d(TAG, "文生图任务完成: ${task.taskId}")
        }.onFailure { error ->
     ...
     * 处理任务失败
     */
    private fun handleTaskFailure(task: AITaskInfo, statusListener: TaskListener) {
        task.status = TaskType.TASK_STATUS_FAILURE
        statusListener.onTaskStatusChanged(task)
        LogUtils.e(TAG, "文生图任务失败: ${task.taskId}")
    }
    ...
    override fun isSupportedTaskType(taskType: Int): Boolean {
        return taskType == TaskType.AI_TYPE_TEXT_TO_IMAGE
    }

    override fun getSupportedTaskTypes(): List<Int> {
        return listOf(TaskType.AI_TYPE_TEXT_TO_IMAGE)
    }
} 