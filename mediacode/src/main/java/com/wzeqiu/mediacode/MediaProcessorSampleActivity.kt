...
import com.common.media.MediaProcessor
    ...
    private val mediaSelectLauncher: ActivityResultLauncher<Intent> = registerForActivityResult(
        ...
        if (result.resultCode == RESULT_OK) {
            result.data?.let { intent ->
                // 获取单个媒体文件结果
                val mediaInfo = intent.getParcelableExtra<MediaInfo>(MediaManageActivity.RESULT_DATA)
                if (mediaInfo != null) {
                    handleSelectedMedia(mediaInfo)
                }
                
                // 获取多个媒体文件结果（如果有）
                val mediaList = intent.getParcelableArrayListExtra<MediaInfo>(MediaManageActivity.RESULT_LIST_DATA)
    ...
    private fun extractAudioFromVideo() {
        ...
        lifecycleScope.launch {
            ...
            runCatching {
                ...
                Toast.makeText(this@MediaProcessorSampleActivity, "音频提取成功", Toast.LENGTH_SHORT).show()
            }.onFailure { e ->
                Log.e(TAG, "提取音频失败", e)
                tvStatus.text = "提取音频失败: ${e.message}"
                Toast.makeText(this@MediaProcessorSampleActivity, "提取音频失败", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun trimVideo() {
        ...
        lifecycleScope.launch {
            ...
            runCatching {
                ...
                tvStatus.text = "视频裁剪成功：${outputFile.name}，时长: ${formatDuration(trimDuration)}"
                Toast.makeText(this@MediaProcessorSampleActivity, "视频裁剪成功", Toast.LENGTH_SHORT).show()
            }.onFailure { e ->
                Log.e(TAG, "裁剪视频失败", e)
                tvStatus.text = "裁剪视频失败: ${e.message}"
                Toast.makeText(this@MediaProcessorSampleActivity, "裁剪视频失败", Toast.LENGTH_SHORT).show()
            }
        }
    }
